...
import asyncio, json, re, os, sys, time, subprocess, tempfile
...
CHANNEL_TYPE = CFG.get('channel_type', 'IG')
...
def save_message(contact_id, contact_name, direction, message):
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            'INSERT INTO dm_conversations (contact_id, contact_name, direction, message, agent_id) VALUES (%s, %s, %s, %s, %s)',
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if r.status_code != 200:
                log(f'Audio download falhou: {r.status_code}')
                return None
        ...
        txt_path = f'/tmp/{os.path.basename(tmp_path).replace(".ogg", ".txt")}'
        text = ''
...
async def send_message(contact_id, message):
    ...
    if SEND_METHOD == 'webhook' and SEND_WEBHOOK_URL:
        ...
        try:
            ...
            data = r.json()
            if not (data.get('messageId') or data.get('id') or data.get('success')):
                log(f'API send response: {str(data)[:200]}')
            return data
        except Exception as e:
...
async def webhook(request: Request):
    ...
    if not message and (attachments or media_url):
        ...
        if not message:
            log(f'Attachment sem texto: {json.dumps(msg_data)[:300]}')
            return {'success': True}
