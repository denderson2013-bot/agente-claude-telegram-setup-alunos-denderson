...
# Sobe drainer da fila local (writes que falharam por tunnel down)
try:
    if getattr(ademir, 'DB_QUEUE_AVAILABLE', False):
        ademir.db_queue.ensure_drainer_running(connect_fn=ademir.db_conn)
        log.info('[bulk-dm] db_queue drainer ativo')
except Exception as _e:
    log.warning(f'[bulk-dm] db_queue drainer falhou: {_e}')
